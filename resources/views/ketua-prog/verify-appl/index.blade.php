...
@section ('main-content')
	...
	<div class="row">
		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			...
			<!-- START: .card .shadow -->
			<div class="card shadow" id="card_verify_appl">
				<div class="card-header">
					<h4>Verify Leave Application</h4>
					<!--
				...
				<div class="card-body">
					...
					<div id="content-bottom">
						<div class="table-responsive">
							<table class="table table-bordered" id="VerApplTbl">	
								<thead>
									<tr>
										...
										@foreach ($list_forms as $lf)
											<tr data-id="{{ $lf->form_id }}">
												...
												<td class="text-center" style="whitespace:nowrap;vertical-align:middle;">
													@if ($lf->form_flag != "LULUS" AND $lf->form_flag != "TOLAK")
														<button type="button" class="btn btn-icon text-primary" title="Edit Application" onclick="edit_appl('{{ $lf->form_id }}')">
															<i class="fas fa-edit"></i>
														</button>
													@endif
													<button type="button" class="btn btn-icon text-success" title="View Leave Application Form" onclick="view_appl('{{ $lf->form_id }}')">
	...
	<script>
		...
		$('#VerApplTbl tbody tr').mouseover(function(){
			$(this).addClass('bg-light');
		}).mouseout(function(){
			$(this).removeClass('bg-light');
		});

		$('#VerApplTbl tbody tr td').not(':last-child').css('cursor', 'pointer');

		$('#VerApplTbl tbody tr td').not(':last-child').on('click', function(){
			var ftid = $($(this).parent()).attr('data-id');
			info_appl(ftid);
		});

		function info_appl(fid) {
			$.get(`/manage/application/info/${fid}`, function (resp){
				if (resp.success){
		...
		function edit_appl(fid){
			$.get(`/verify/application/edit/${fid}`, function(resp){
				if (resp.success){
					var ori_card_header = $('#card_verify_appl .card-header').html();
					var close_btn = $(
					...
					$(close_btn).on('click', function(e){
						$('#card_verify_appl .card-header').empty().html(ori_card_header);
						$('#card_verify_appl #content-top').slideUp();
						$('#card_verify_appl #content-bottom').slideDown();
						$('.section').load('/verify/application');
					});
					$('#card_verify_appl .card-header')
						.empty()
						.append('<h4>Edit Application</h4>')
						.append($('<div class="card-header-action"></div>').append(close_btn));
					$('#card_verify_appl #content-top').empty().html(resp.view_temp);
					$('#card_verify_appl #content-top').slideDown();
					$('#card_verify_appl #content-bottom').slideUp();
				}
				else{
					pop_swal('warning', resp.msg);
				}
		...
		function view_appl(fid){
			swal("Info!", "Function `View Leave Application Form` Still In Progress!", "info");
		}
	</script>
@endsection
